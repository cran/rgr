...
function(group, x, file = NULL)
{
     ...
     # in the R working directory.  The file name prefix should identify the 
     # data source and framework group, e.g., "Project3", the group name and 
     # variable name are appended together with ".csv" prior to opening a 
     # file for the results; these can be opened/viewed with MS Excel. 
     ...
     # attach(dfname) and at close, if necesary, detach(dfname).
     #
     ...
     if (is.null(file)) {
         ...
         cat("  By default the output file will be saved in the R working directory as:\n",
             ...
             "  \"_groupname_xname.csv\" will be appended, set, for example:\n",
             "  file = \"D://R_work//Project3//C_soils\"\n",
             "  If no directory is specified, the file will be saved in the R working directory\n\n")
         filename <- paste(wdname, "/" ,dfname, "_", groupname, "_", xname, ".csv", sep = "")
     }
     else filename <- paste(file, "_", groupname, "_", xname, ".csv", sep = "")  
     #
