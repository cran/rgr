gx.rma <-
function (xx1, xx2, x1lab = NULL, x2lab = NULL, 
    log = FALSE, if.plot = FALSE, if.rma = FALSE, ...) 
{
    if (!if.plot) if.rma <- FALSE
    if (is.matrix(xx1)) {
    ...
    if(is.null(x1lab)) x1lab <- xx1.name
    if(is.null(x2lab)) x2lab <- xx2.name
    ...
    xsdv <- sqrt(xvar)
    slope <- xsdv[2]/xsdv[1]
# Fix to avoid using function cor, as cor can be masked by package NADA
    ...
    cat("\n\n Means =\t", signif(xbar[1], 4), "\t\t", signif(xbar[2], 4),
        ...
        "\n\n Corr =\t\t", round(r, 4), "\n N =\t\t", xlen,
        "\n\t\t\t\t   SE\t\t\t95% CLs", 
        "\n Slope =\t", signif(slope, 4), "\t", signif(seslp, 
            4), "\t  ", signif(slpll, 4), "<->", signif(slpul, 
            4), "\n Intercept =\t", signif(incpt, 4), "\t", signif(seint, 
            6), "\t  ", signif(intll, 4), "<->", signif(intul, 
            4), "\n\n")
    if(if.rma) abline(incpt, slope, lty = 3, col = 2)
