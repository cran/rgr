...
function (x, by, log = FALSE, logx = NULL, notch = TRUE, xlab = "", 
    ...
    cex = 1, adj = 0.5, add = FALSE, ssll = 1, colr = 8, ...) 
{
    if (is.matrix(x)) 
        data <- data.frame(x)
    ...
    if (!add) {
        if (is.null(ylim)) 
            plot(range(c(xpos - (0.5 * width)/space, xpos + (0.5 * 
                width)/space)), range.data, log = logplot, type = "n", 
                xaxt = "n", ylab = ylab, xlab = xlab, main = main, 
                ...)
        else plot(range(c(xpos - (0.5 * width)/space, xpos + 
            (0.5 * width)/space)), range.data, log = logplot, 
            type = "n", xaxt = "n", ylab = ylab, xlab = xlab, 
            ylim = ylim, main = main, ...)
    }
    xmax <- 10^12
    for (i in 1:cols) {
    ...
    axis(1, xpos, label, tick = FALSE, las = las, cex.axis = cex, 
        adj = adj)
    invisible()
}
